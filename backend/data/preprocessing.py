...
# Uses keyword-based classification that looks at the full organization name,
# not just the prefix. This produces meaningful, non-overlapping categories:
# e.g., a "БУ ... БОЛЬНИЦА" is classified as Hospital, not Budget Institution.

def extract_institution_type(name):
    """Classify institution by analyzing the full Russian organization name."""
    if not isinstance(name, str):
        return 'other'
    n = name.strip().upper()

    # Kindergarten / Preschool
    for kw in ['МБДОУ', 'МАДОУ', 'МКДОУ', 'ДЕТСКИЙ САД', 'Д/С', 'ДОУ']:
        if kw in n:
            return 'kindergarten'

    # School (municipal + state)
    for kw in ['МБОУ', 'МАОУ', 'МКОУ', 'ГБОУ', 'СОШ', 'ШКОЛА', 'ЛИЦЕЙ',
               'ГИМНАЗИЯ', 'ШКОЛА-ИНТЕРНАТ']:
        if kw in n:
            return 'school'
    ...
    # Hospital / Healthcare (checked broadly by medical keywords)
    for kw in ['БОЛЬНИЦ', 'ГОСПИТАЛ', 'ПОЛИКЛИНИК', 'МЕДИЦИН', 'МИНЗДРАВ',
               'ДИСПАНСЕР', 'ПЕРИНАТАЛЬН', 'РОДДОМ', 'РОДИЛЬН', 'САНАТОРИ',
               'ГБУЗ', 'ГАУЗ', 'ГКУЗ', 'КГБУЗ', 'ТОГБУЗ', 'ОГБУЗ', 'ГОБУЗ',
               'ЦРБ', 'РБ ', 'ГКБ', 'КЛИНИК', 'ХИРУРГ', 'ОНКОЛ', 'КАРДИО',
               'РЕАБИЛИТАЦ', 'ЗДРАВООХРАН', 'ПСИХО', 'СТОМАТ', 'ОФТАЛЬМ']:
        if kw in n:
            return 'hospital'
    if 'ФГБУ' in n:
        return 'hospital'
    if re.match(r'^["\s«]*БУ[\s"]', n):
        return 'hospital'

    # Social Services
    for kw in ['СОЦИАЛЬН', 'СРЦДН', 'ПРИЮТ', 'ИНТЕРНАТ', 'ДЕТСКИЙ ДОМ',
               'ЦЕНТР СОЦИАЛ', 'ДОМ РЕБЕНКА', 'ДОМ-ИНТЕРНАТ', 'ПАНСИОНАТ',
               'ЦСОН', 'СРЦ']:
        if kw in n:
            return 'social_services'

    # Catering / Food Services
    for kw in ['КОМБИНАТ ПИТАНИЯ', 'ШКОЛЬНОГО ПИТАНИЯ', 'СТОЛОВАЯ']:
        if kw in n:
            return 'catering'

    # Youth & Culture Centers
    for kw in ['МОЛОДЕЖ', 'ЮНОШЕСК', 'СПОРТ', 'ДЮЦ', 'ДОСУГ', 'КУЛЬТУР',
               'ТВОРЧЕСТВ']:
        if kw in n:
            return 'youth_culture'

    # Government Agencies
    for kw in ['ФКУ', 'УФСИН', 'МВД', 'ФСИН', 'ПОЛИЦ', 'ВОЕНН', 'ОБОРОН']:
        if kw in n:
            return 'government'
